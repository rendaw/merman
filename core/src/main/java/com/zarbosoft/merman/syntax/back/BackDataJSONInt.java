...
@Configuration(name = "json_data_int")
public class BackDataJSONInt extends BackPart {
	@Configuration
	public String middle;

	@Override
	public Node buildBackRule(final Syntax syntax, final AtomType atomType) {
		return new Operator(new MatchingEventTerminal(new JIntEvent(null)), store -> {
			store = (Store) store.pushStack(new Pair<>(middle,
					new ValuePrimitive(atomType.getDataPrimitive(middle), ((JIntEvent) store.top()).value)
			));
