...
public class TestPrimitiveBreaking {
	...
	public void testUnbreakCursor() {
		...
		new GeneralTestWizard(PrimitiveSyntax.syntax, primitiveAtom)
				...
				.checkCourse(0, -10, 0)
				.run(context -> {
					assertThat(primitive.visual.selection.range.cursor.drawing.transverse(context), is(-13));
					assertThat(primitive.visual.selection.range.cursor.drawing.transverseEdge(context), is(1));
				});
	}
	...
	public void testUnbreakClear() {
		final Atom primitiveAtom = new TreeBuilder(PrimitiveSyntax.primitive).add("value", "word egg").build();
		final ValuePrimitive primitive = (ValuePrimitive) primitiveAtom.data.get("value");
		new GeneralTestWizard(PrimitiveSyntax.syntax, primitiveAtom)
				.resize(40)
				.checkCourseCount(2)
	...
	public void testUnbreakClearEnd() {
		final Atom primitiveAtom = new TreeBuilder(PrimitiveSyntax.primitive).add("value", "gate\nword egg").build();
		final ValuePrimitive primitive = (ValuePrimitive) primitiveAtom.data.get("value");
		new GeneralTestWizard(PrimitiveSyntax.syntax, primitiveAtom)
				.resize(40)
				.checkCourseCount(3)
	...
	public void testUnbreakClearStart() {
		final Atom primitiveAtom = new TreeBuilder(PrimitiveSyntax.primitive).add("value", "word egg\nroad").build();
		final ValuePrimitive primitive = (ValuePrimitive) primitiveAtom.data.get("value");
		new GeneralTestWizard(PrimitiveSyntax.syntax, primitiveAtom)
				.resize(40)
				.checkCourseCount(3)
	...
	public void testAddThenSplit() {
		final Atom primitiveAtom = new TreeBuilder(PrimitiveSyntax.primitive).add("value", "ab").build();
		final ValuePrimitive primitive = (ValuePrimitive) primitiveAtom.data.get("value");
		new GeneralTestWizard(PrimitiveSyntax.syntax, primitiveAtom)
				.resize(40)
				.checkCourseCount(1)
